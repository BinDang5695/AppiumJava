...
public class ExcelHelpers {
    ...
    public Object[][] getExcelData(String excelPath, String sheetName, int startRow, int endRow) {
        Object[][] data = null;
        Workbook workbook = null;
        ...
        try {
            ...
            if (sheet == null) {
                throw new RuntimeException("❌ Sheet '" + sheetName + "' not found in file: " + excelPath);
            }
            ...
            // Kiểm tra giới hạn startRow / endRow
            if (startRow < 1 || startRow >= totalRows) {
                throw new IllegalArgumentException("StartRow must be between 1 and " + (totalRows - 1));
            }
            if (endRow < startRow || endRow >= totalRows) {
                throw new IllegalArgumentException("EndRow must be between StartRow and " + (totalRows - 1));
            }

            System.out.println("Sheet total rows: " + totalRows + ", total columns: " + totalCols);

            // Khởi tạo mảng dữ liệu
            data = new Object[(endRow - startRow) + 1][totalCols];
            Cell cell;

            // Vòng lặp đọc dữ liệu theo khoảng dòng
                    ...
                    switch (cell.getCellType()) {
                        ...
                        case NUMERIC:
                            if (DateUtil.isCellDateFormatted(cell)) {
                                data[i - startRow][j] = cell.getDateCellValue().toString();
                            } else {
                                data[i - startRow][j] = String.valueOf(cell.getNumericCellValue());
                            }
                            break;
                        case BOOLEAN:
                            data[i - startRow][j] = String.valueOf(cell.getBooleanCellValue());
                            break;
                        case BLANK:
                            data[i - startRow][j] = "";
                            break;
                        default:
                            data[i - startRow][j] = cell.toString();
                            break;
                    }
                }
            }
