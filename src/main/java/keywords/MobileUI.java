...
public class MobileUI {
    ...
    public static void swipeElementLeftToRight(By locator, int duration) {
        sleep(STEP_ACTION_TIMEOUT);
    ...
    public static void swipeElementRightToLeft(By locator, int duration) {
        sleep(STEP_ACTION_TIMEOUT);
        WebElement element = DriverManager.getDriver().findElement(locator);
    ...
    public static void doubleTap(int x, int y) {
        ...
        sequence.addAction(
                new Pause(finger, Duration.ofMillis(100)));
    ...
    public static void drawLine(int x1, int y1, int x2, int y2) {
        sleep(STEP_ACTION_TIMEOUT);
        LogUtils.info("[MobileUI] Drawing line from (" + x1 + "," + y1 + ") to (" + x2 + "," + y2 + ")");

        PointerInput finger = new PointerInput(PointerInput.Kind.TOUCH, "finger");

        Sequence draw = new Sequence(finger, 1);

        draw.addAction(finger.createPointerMove(
                Duration.ZERO,
                PointerInput.Origin.viewport(),
                x1, y1
        ));

        draw.addAction(finger.createPointerDown(
                PointerInput.MouseButton.LEFT.asArg()
        ));
        ...
        draw.addAction(finger.createPointerUp(
                PointerInput.MouseButton.LEFT.asArg()
        ));

        getDriver().perform(Collections.singletonList(draw));
    }
    ...
    public static void zoom(WebElement element, double scale) {
        ...
        if (scale > 1) { // Move fingers further apart
    ...
    public static void zoom(By locator, double scale) {
        sleep(STEP_ACTION_TIMEOUT);
        LogUtils.info("[MobileUI] Executing zoom on locator: " + locator + " with scale: " + scale);
    ...
    public static void scrollVertical(double startPercent, double endPercent, int duration) {
        sleep(STEP_ACTION_TIMEOUT);
        Dimension size = getDriver().manage().window().getSize();
        ...
        scroll(x, startY, x, endY, duration);
    }

    public static void scrollDown() {
        scrollVertical(0.85, 0.30, 1500);
    }

    public static void scrollUp() {
        scrollVertical(0.25, 0.75, 1000);
    }
    ...
    @Step("Get text from element {0}")
    public static String getElementText(By locator, String expected, String message) {
    ...
    public static void verifyText(String actualText, String expectedText, String message) {
        LogUtils.info("[MobileUI] Verifying text: '" + actualText + "' equals '" + expectedText + "'");
    ...
    public static void verifyElementContentDesc(WebElement element, String expected, String message) {
        waitForElementVisible(element);
    ...
    public static void verifyEquals(String actual, String expected, String message) {
        LogUtils.info("[MobileUI] Verifying equals. Actual: '" + actual + "', Expected: '" + expected + "'");
        Assert.assertEquals(actual, expected, message);
    }
    ...
    public static void hideKeyboard() {
        try {
            ...
            LogUtils.info("[MobileUI] Hide keyboard");
        } catch (Exception e) {
            LogUtils.warn("[MobileUI] Keyboard not visible");
        }
    }

    public static void longPress(int x, int y, int durationMs) {
        sleep(STEP_ACTION_TIMEOUT);
        LogUtils.info("[MobileUI] Long press at (" + x + "," + y + ") for " + durationMs + "ms");
    ...
    public static void longPress(By locator, int durationMs) {

        sleep(STEP_ACTION_TIMEOUT);
        WebElement element = getDriver().findElement(locator);

        LogUtils.info("[MobileUI] Long press on element: " + locator);
